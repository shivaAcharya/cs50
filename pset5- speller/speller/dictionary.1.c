...
bool check(const char *word)
{
    ...
    // Use tmp cursor pointer, point to 1st element, if not cursor = cursor -> next;
    // Stop when found word or cursor points to NULL.
...
bool load(const char *dictionary)
{
    ...
    while (fscanf(f, "%s", lexicon) != EOF)
    {
        ...
        // Use malloc/calloc
        // Remember to check if return value in NULL
        ...
        if (n != NULL)
        {
            ...
            // Be sure to set pointer in the correct order
            if (!table[hash_code])
            {
...
bool unload(void)
{
    ...
    // 3. Free tmp, free (tmp), update tmp to point to second node, tmp = cursor
